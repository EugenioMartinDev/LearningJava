...
public class GestorLibreria {
	...
	/**
	 * Método que incluye un libro en la libreria, con las restricciones:
	 * - El atributo isbn de libro no es nulo ni vacío y es único
	 * - El atributo titulo de libro no es nulo ni vacío.
	 * @param libro : libro a incluir
	 * @param libreria : librería repositorio
	 * @return : Devuelve una indicación de si se acepta o se rechaza el alta del libro.
	 */
	public boolean altaLibro(Libro libro) {
		
		if (libro.getIsbn() != null && libro.getIsbn().length() > 0
				&& libro.getTitulo() != null && libro.getTitulo().length() > 0)
		{
			if (isbnUnico(libro)) {
	...
	/**
	 * Método interno que decide si ya existe un libro con el mismo isbn en la librería
	 * @param libro
	 * @param libreria
	 * @return
	 */
	...
	/**
	 *  Método que busca todos los libros de una editorial. Puede ser un array vacío si no se encuentran libros.
	 * @param libreria : librería repositorio
